...
employeeController.clockIn = (req, res, next) => {
  ...
  const queryText =
    'INSERT INTO timesheet (clock_in, week, emp_id) VALUES (($1), ($2), ($3)) RETURNING entry_id;';
...
employeeController.clockOut = (req, res, next) => {
  ...
  db.query(queryText, values)
    .then((response) => {
      ...
      const addToTable =
        'UPDATE timesheet SET clock_out = ($1), hours = ($2) WHERE entry_id = ($3);';
...
employeeController.authorize = (req, res, next) => {
  ...
  // define query text
  const queryText =
    'SELECT employee_type, first_name, emp_id FROM all_employees WHERE username=($1) AND password=($2);';
...
employeeController.getHours = (req, res, next) => {
  ...
  const queryText =
    'SELECT sum(hours) FROM timesheet WHERE emp_id=($1) AND week = 6;';
  const values = [emp_id];
  db.query(queryText, values)
    .then((response) => {
...
employeeController.getUsers = (req, res, next) => {
  const queryText =
    'SELECT ts.emp_id, ae.first_name, ae.last_name, SUM(hours) as hours_worked FROM timesheet as ts INNER JOIN all_employees AS ae ON ts.emp_id=ae.emp_id GROUP BY ae.first_name, ts.emp_id, ae.last_name;';
  db.query(queryText)
    .then((response) => {
